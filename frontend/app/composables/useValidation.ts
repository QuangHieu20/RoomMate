...
export const useValidationSchema = () => {
  ...
  const postFormSchema = computed(() => yup.object({
    // Location fields
    province: yup
      .string()
      .required(t('post.validation.provinceRequired')),
    district: yup
      .string()
      .required(t('post.validation.districtRequired')),
    ward: yup
      .string()
      .required(t('post.validation.wardRequired')),
    address: yup
      .string()
    ...
    roomType: yup
      .string()
      .required(t('post.validation.roomTypeRequired')),
    price: yup
      .number()
    ...
    priceType: yup
      .string()
      .required(t('post.validation.priceTypeRequired')),
    area: yup
      .number()
    ...
    contactName: yup
      ...
      .test('min-length', t('post.validation.contactNameMin'), function(value) {
        if (value === null || value === undefined || value === '') return true
        return value.length >= 2
      })
      .test('max-length', t('post.validation.contactNameMax'), function(value) {
        if (value === null || value === undefined || value === '') return true
    ...
    contactPhone: yup
      ...
      .test('phone-format', t('post.validation.contactPhoneInvalid'), function(value) {
        if (value === null || value === undefined || value === '') return true
        return /^[0-9]+$/.test(value)
      })
      .test('phone-min', t('post.validation.contactPhoneMin'), function(value) {
        if (value === null || value === undefined || value === '') return true
        return value.length >= 8
      })
      .test('phone-max', t('post.validation.contactPhoneMax'), function(value) {
        if (value === null || value === undefined || value === '') return true
    ...
    mediaFiles: yup
      ...
      .test('total-size', t('post.validation.totalSizeMax'), function(files) {
      ...
      .test('video-count', t('post.validation.videoCountMax'), function(files) {
        if (!files || files.length === 0) return true
        // Kiểm tra số lượng video (base64 bắt đầu bằng video MIME type)
        const videoCount = files.filter(base64 => 
          base64 && base64.startsWith('data:video/')
        ).length
        return videoCount <= 1
      })
      .test('media-type', 'Chỉ chấp nhận file ảnh hoặc video', function(files) {
        if (!files || files.length === 0) return true
  ...
  const findRoomSchema = computed(() => yup.object({
