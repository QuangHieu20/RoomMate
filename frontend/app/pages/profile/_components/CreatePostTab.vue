...
<template>
  <div class="card bg-base-100 shadow-sm border border-base-300 mb-6">
    <div class="card-body p-4 lg:p-6">
      ...
      <div
        role="tablist"
        class="tabs tabs-bordered w-full border-b border-gray-200 mb-6"
      >
        <a
          role="tab"
          @click="$emit('update:createPostTab', 'rent-out')"
          :class="[
            'tab flex-1 text-base font-medium',
            createPostTab === 'rent-out'
              ? 'tab-active text-cyan-600 !border-b-cyan-500 !border-b-2'
              : 'text-gray-600',
          ]"
        >
      ...
      >
        ...
        <div>
          ...
          <div class="card bg-base-100 shadow-sm border border-base-300 mb-2.5">
            <div class="card-body p-4">
              <h3 class="card-title">
                {{ $t('post.create.sections.location') }}
              </h3>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <!-- Tỉnh/thành phố -->
                <div>
                  <VeeField v-slot="{ field, errorMessage }" name="province">
                    <BaseSelect
                      ...
                      :placeholder="
                        $t('post.create.placeholders.selectProvince')
                      "
                      :required="true"
                      :error="!!errorMessage"
                ...
                <!-- Quận/huyện -->
                <div>
                  <VeeField v-slot="{ field, errorMessage }" name="district">
                    <BaseSelect
                      :model-value="field.value"
                      :name="field.name"
                      @update:model-value="field.onChange"
                      ...
                      :placeholder="
                        $t('post.create.placeholders.selectDistrict')
                      "
                      :required="true"
                      :error="!!errorMessage"
                      :error-message="errorMessage"
              ...
              <!-- Phường/xã -->
              <div class="mb-4 w-[48%]">
                <VeeField v-slot="{ field, errorMessage }" name="ward">
                  <BaseSelect
                    :model-value="field.value"
                    :name="field.name"
                    @update:model-value="field.onChange"
          ...
          <div class="card bg-base-100 shadow-sm border border-base-300 mb-2.5">
            <!-- Loại phòng -->
            <div class="card-body p-4">
              <VeeField v-slot="{ field, errorMessage }" name="roomType">
                <BaseSelect
                  :model-value="field.value"
                  :name="field.name"
                  @update:model-value="field.onChange"
            ...
            <div class="card-body p-4">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Giá -->
                <div>
                  <VeeField v-slot="{ field, errorMessage }" name="price">
                    <BaseInput
                      :model-value="field.value"
                      :name="field.name"
                      @update:model-value="field.onChange"
                      ...
                      class="w-full !bg-[#F7FAFC]"
                    >
                      <template #subTitle> Triệu/tháng </template>
                    </BaseInput>
                  </VeeField>
                </div>
                ...
                <!-- Diện tích -->
                <div>
                  <VeeField v-slot="{ field, errorMessage }" name="area">
                    <BaseInput
                      :model-value="field.value"
                      :name="field.name"
                      @update:model-value="field.onChange"
          ...
          <div class="card bg-base-100 shadow-sm border border-base-300 mb-2.5">
            <div class="card-body p-4">
              <h3 class="card-title">
                {{ $t('post.create.sections.description') }}
              </h3>
              <div class="mb-4">
                <VeeField v-slot="{ field, errorMessage }" name="title">
                  <BaseInput
                    :model-value="field.value"
                    :name="field.name"
                    @update:model-value="field.onChange"
              ...
              <div class="mb-4">
                <VeeField v-slot="{ field, errorMessage }" name="description">
                  <BaseInput
                    ...
                    :placeholder="
                      $t('post.create.placeholders.enterDescription')
                    "
                    :required="true"
                    :maxlength="5000"
                    :rows="6"
          ...
          <div class="card bg-base-100 shadow-sm border border-base-300 mb-2.5">
            <div class="card-body p-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Tên liên hệ -->
                <div>
                  <VeeField v-slot="{ field, errorMessage }" name="contactName">
                    <BaseInput
                      :model-value="field.value"
                      :name="field.name"
                      @update:model-value="field.onChange"
                      ...
                      :placeholder="
                        $t('post.create.placeholders.enterContactName')
                      "
                      :error="!!errorMessage"
                      :error-message="errorMessage"
                      class="w-full !bg-[#F7FAFC]"
                ...
                <div>
                  ...
                  >
                    <BaseInput
                      ...
                      :placeholder="
                        $t('post.create.placeholders.enterContactPhone')
                      "
                      :error="!!errorMessage"
                      :error-message="errorMessage"
                      class="w-full !bg-[#F7FAFC]"
              ...
              <div class="mb-4">
                ...
                <div class="flex flex-wrap gap-4 items-start">
                  ...
                  <div class="flex flex-col items-center space-y-2">
                    <div
                      class="w-20 h-20 bg-gray-200 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-300 hover:border-cyan-400 transition-colors cursor-pointer"
                    >
                      <input
                        type="file"
                        multiple
                  ...
                  >
                    ...
                    <div class="relative group">
                      <div
                        class="w-20 h-20 rounded-lg overflow-hidden bg-gray-100 border-2 border-gray-200 hover:border-cyan-400 transition-all duration-200 shadow-sm hover:shadow-md"
                      >
                        <!-- Image preview -->
                        <img
                        ...
                        >
                          <span class="text-2xl">{{
                            getFileIcon(uploadedFiles[index])
                          }}</span>
                        </div>
                      </div>
                ...
                <p class="text-sm text-gray-600 mt-4">
                  Tối đa 5 ảnh/video với tin đăng. Tổng dung lượng không quá
                  8MB. Chỉ được upload tối đa 1 video.
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          >
            <span
              v-if="loading || createPostLoading"
              class="loading loading-spinner loading-sm"
            ></span>
            {{
              loading || createPostLoading
                ? $t('post.create.buttons.submitLoading')
                : $t('post.create.buttons.submit')
            }}
          </button>
        </div>
      </VeeForm>
      ...
      >
        ...
        <div>
          ...
          <div>
            ...
            <div class="mb-4">
              <VeeField v-slot="{ field, errorMessage }" name="description">
                ...
                <ClientOnly>
                  <div class="text-right text-sm text-gray-500 mt-1">
                    {{ values.description?.length || 0 }}/5000
          ...
          <div class="mt-6">
            <h3 class="card-title">
              {{ $t('post.create.sections.requirements') }}
            </h3>
