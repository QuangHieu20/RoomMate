    ...
    >
      <div class="py-1">
        <button
          ...
          :class="{ 'bg-blue-50 text-blue-700': locale.code === currentLocale?.code }"
        >
          <span class="text-lg">{{ getFlag(locale.code) }}</span>
          <span>{{ locale.name }}</span>
          <svg v-if="locale.code === currentLocale?.code" class="w-4 h-4 ml-auto" fill="currentColor" viewBox="0 0 20 20">
...
const switchLocale = async (localeCode: string) => {
  try {
    ...
    const { setLocale: setVeeValidateLocale } = await import('@vee-validate/i18n')
