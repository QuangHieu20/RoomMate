...
<template>
  <div class="bg-white h-full flex flex-col">
    ...
    <!-- Chat List -->
    <div class="flex-1 overflow-y-auto divide-y divide-gray-200 min-h-0 overflow-auto">
      <div 
        v-for="chat in chats" 
        :key="chat.id"
        @click="selectChat(chat)"
        class="p-4 hover:bg-gray-50 cursor-pointer transition-colors"
        :class="{ 'bg-cyan-50 border-r-4 border-cyan-500': selectedChat?.id === chat.id }"
      >
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
              <h3 class="font-medium text-gray-900 truncate">{{ chat.user.name }}</h3>
              <span class="text-xs text-gray-500">{{ chat.lastMessage.time }}</span>
            </div>
            <p class="text-sm text-gray-600 truncate mt-1">{{ chat.lastMessage.text }}</p>
          </div>

          <!-- Unread Badge -->
          <div v-if="chat.unreadCount > 0" class="flex-shrink-0">
            <span class="bg-cyan-500 text-white text-xs rounded-full px-2 py-1 min-w-[20px] text-center">
